...
def new(name='sha512', nonce=False, entropy=False):
    ''' Returns a new DRBG.
    ...
    if digest:
        ...
        entropy = entropy if entropy else os.urandom(32)
        ...
        nonce = nonce if nonce else os.urandom(drbg.seedlen)
        entropy = False
        drbg.init(nonce)
    
    return RandomByteGenerator(drbg), nonce, entropy
